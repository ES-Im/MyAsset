...
| user_key    | 사용자의 내부식별자(category:user = N:1) null이면 시스템 디폴트 카테고리 사용 |
...
| user_key     | 사용자의 내부식별자(alarm:user = N:1) null시 시스템 디폴트 설정을 뜻함  |
| notiType     | 알림원인                                               |
| expenseType  | 대상 지출 타입 (변동/고정/적금)                                |
| isActive     | 디폴트 = true / false시 해당 알림을 보내지 않는다                 |
...
### 정책
    - user_key = null : 알림규칙의 시스템 디폴트 규칙
    - user_key != null : 알림규칙의 사용자 커스텀 규칙
        - 커스텀 대상 : notiType 중에서 알림 활성화/비활성화 선택
    - notificationRule은 notification이 create될때 기준이 된다.
        - isActive = false 인 notiType 알림은 생성되지 않는다.
